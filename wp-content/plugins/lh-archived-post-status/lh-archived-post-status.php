...
class LH_archived_post_status_plugin {
    ...
    static function get_applicable_post_types() {
        ...
        $posttypes = array_diff($posttypes, $excludable_post_types);

        $posttypes = apply_filters( 'lh_archive_post_status_posttypes_filter', $posttypes );
        $posttypes = apply_filters( 'lh_archive_post_status_get_applicable_post_types_filter', $posttypes );
    ...
    static function setup_crons(){
        ...
        wp_schedule_event( time() + wp_rand( 0, 1000 ), 'hourly', 'lh_archived_post_status_run' );
    ...
    public function render_archive_date_box_content( $post, $callback_args ){
        ...
        ?><tr valign="top"><?php
        ?><th scope="row"><label for="<?php echo self::return_plugin_namespace(); ?>-post_expires"><?php _e("Archive Date", self::return_plugin_namespace() ); ?></label></th><?php
        ?><td><?php
        ?><input type="date" name="<?php echo self::return_plugin_namespace(); ?>-post_expires" id="<?php echo self::return_plugin_namespace(); ?>-post_expires" value="<?php if (!empty($raw_date)) { echo date('Y-m-d', $archive_date); } ?>" /><?php
        ?></td><?php
        ?></tr><?php
        ?></table><?php
    ...
    public function render_title_label_input($args) { 
        ...
        ?><input type="text" name="<?php echo self::return_opt_name().'['.$args[0].']'; ?>" id="<?php echo $args[0]; ?>" value="<?php echo $value; ?>" size="20" /><br/><?php _e("This label will appear after the title for archived posts on the front end of your website", self::return_plugin_namespace()); ?><?php
    ...
    public function modify_title($title, $post_id = NULL){
        ...
        if (in_the_loop() && is_singular() && isset($post_id) and is_numeric($post_id)){
        ...
        if (($post->post_status == "publish") && self::is_applicable_post_type($post->post_type)){
            ...
            if ( current_user_can('edit_post', $post->ID) ) {
                ...
                if ( current_user_can('publish_posts') ) {
                    ...
                    $actions['archive_link']  = '<a href="'.$url.'" title="' . esc_attr( __( 'Archive this post' , self::return_plugin_namespace()) ) . '">' . __( 'Archive', self::return_plugin_namespace() ) . '</a>';
    ...
    public function plugin_init(){
        ...
        */
        ...
        add_filter('lh_wayback_machine_post_status_filter', array($this,'maybe_make_waybackable'), 10, 1);
